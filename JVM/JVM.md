...
<img src = "./img/JVM堆结构.png" width = 80% height = 80%>

### 进入老年代

**提升**

每触发一次Minor GC，存活的对象年龄就会+1，当达到阈值就会把这些对象放入老年代。这个阈值，可以通过参数 ‐XX:+MaxTenuringThreshold 进行配置。

**分配担保**

每次Minor GC时，存活对象都会放入其中一个survivor区，当这个区不够放时，就会直接把不够放的对象放入老年代。

**大对象直接在老年代分配**

超出某个大小的对象将直接在老年代分配。这个值是通过参数 -XX:PretenureSizeThreshold 进行配置的。默认为 0，意思是全部首选 Eden 区进行分配。

> 大对象如果分配在Eden区，每次Minor GC时标记-复制效率太低。

**动态判断对象年龄**

survivor区中 **累加**小于等于某age的对象大小的和，大于survivor区的一半，≥该age的对象直接进入老年代。

[动态对象年龄判定](https://blog.csdn.net/u014493323/article/details/82921740)
...
- 标记-复制
  ...
  将内存分为大小相同的两块A和B，每次使用其中一块A，每次GC时把存活对象移动到未被使用的那块B，回收A。

  空间利用率较低，总是只使用50%的内存。

- 标记-整理

  让所有存活对象像内存一段移动，回收端边界之外的内存。

- 分代收集

  根据对象存活周期的特性不同将内存分为几块。

  - 新生代

    新生代每次GC都有大量对象死去，所以使用”标记-复制“算法，可以只用少量额外的空间即可完成GC。

    <img src = "./img/年轻代.png" width = 60% height = 60%>

    当**Eden区分配满**时，就会触发Minorr GC：

    - Eden区执行GC，存活对象复制到其中一个survivor区(以S0举例)。

    - 当Eden区再次GC，将Eden和S0同时”标记-复制“，存活的放入S1，清理Eden和S0。

    - 当Eden区再次GC，将Eden和S1同时”标记-复制“，存活的放入S0，清理Eden和S1。

      在整个过程中，总有一个survivor区是空置的。一般为8:1:1，因此只浪费10%。

  - 老年代

    老年代每次GC的对象存活率较高，且没有额外空间进行分配担保，使用”标记-整理“。
